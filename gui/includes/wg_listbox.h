...
public:
	...
	SListItem(std::string sItemTextIn, void* pItemDataIn = 0, CRGBColor ItemColorIn = DEFAULT_TEXT_COLOR) :
...
public:
	...
	CListBox(const CRect& WindowRect, CWindow* pParent, bool bSingleSelection = false, unsigned int iItemHeight = 12, CFontEngine* pFontEngine = 0);
	...
	int AddItem(SListItem ListItem);
	...
	SListItem& GetItem(int iItemIndex) { return m_Items.at(iItemIndex); }
	...
	bool IsSelected(int iItemIndex)
		{ return (iItemIndex >= 0 && iItemIndex < stdex::safe_static_cast<int>(m_SelectedItems.size()) && m_SelectedItems.at(iItemIndex)); }
